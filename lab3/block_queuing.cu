...
__global__ void block_queuing_kernel(int block_queue_capacity){
	int i = threadIdx.x + (blockIdx.x * blockDim.x);
	int* shared_mem_queue = (int*)malloc(block_queue_capacity * sizeof(int));
	...
	for (int idx = i; idx < numCurrLevelNodes; idx++) {
		...
		for (int nbrIdx = nodePtrs_h[node]; nbrIdx < nodePtrs_h[node + 1]; nbrIdx++) {
			...
			if (!nodeVisited_h[neighbor]) {
				...
				nextLevelNodes_h[numNextLevelNodes_h] = neighbor; //replace that line by adding to shared_mem_queue instead
				if (numNextLevelNodes_h == block_queue_capacity - 1) { //queue full
...
int process_block(int argc, char* argv[]) {
	...
	global_queuing_kernel << < num_of_blocks, num_of_threads >> > (block_queue_capacity);
